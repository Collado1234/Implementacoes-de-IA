...
class DecisionTreeClassifier:
    def __init__(self,
                 ...
                 max_features: Optional[int] = None): # Novo parâmetro
    ...
    def _best_split(self, X: np.ndarray, y: np.ndarray) -> Tuple[Optional[int], Optional[float], float]:
        ...
        if self.max_features is None:
            # Se max_features não foi passado (árvore normal), usa todas as features
            feature_indices_to_consider = range(n)
        else:
            # Se max_features foi passado (usado pelo Random Forest), 
            # seleciona aleatoriamente o número de features especificado.
            # np.arange(n) cria [0, 1, 2, ..., n-1]
            # np.random.choice seleciona 'self.max_features' elementos sem reposição.
            num_features_to_use = min(self.max_features, n)
            feature_indices_to_consider = np.random.choice(
                np.arange(n), size=num_features_to_use, replace=False
            )
        for feature_index in feature_indices_to_consider: # <-- Iteraçãomodificada
